...
class JsProxyizeTransformer extends Transformer with ResolverTransformer {
  ...
  Future applyResolver(Transform transform, Resolver resolver) async {
    var library = resolver.getLibraryByName('jsproxyize_trasformer.metadata');

    if (library == null) {
      transform.addOutput(transform.primaryInput);
      return;
    }
    print('Visiting ${transform.primaryInput.id.path}');
    print('I know what a JsProxyize is!');
    ...
    for (var directive in unit.directives) {
      if (directive is ImportDirective && directive.uri.stringValue == 'package:io_2016_contacts_demo/transformer/metadata.dart') {
